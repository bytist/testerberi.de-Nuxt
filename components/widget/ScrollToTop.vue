...
export default {
    ...
    mounted() {
        window.smoothscroll = () => {
            ...
            if (currentScroll > 0 && !this.inScroll) {
                window.requestAnimationFrame(window.smoothscroll);
                window.scrollTo(
                    0,
                    Math.floor(currentScroll - (currentScroll / 10 > 300 ? 90 : currentScroll / 10))
                    // Math.floor(currentScroll - currentScroll / (this.frame - currentScroll / 1000))
                );
            }
        };
